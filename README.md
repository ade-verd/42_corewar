...
| **`ld`**<br /><br />*`DIR`&#124;`IND`, `REG`* |This instruction `ld` takes 2 parameters. The value of the first parameter (which is a `DIR` or an `IND`) is loaded into the second parameter which is a `REG`. Example: `ld 34,r3` stores `REG_SIZE` octets from 34 bytes after the current address (`(PC + (34 % IDX_MOD))`) into the register `r3`.| 5 | Oui | Oui | 4 |
