...
| 4 | `add` | `REG`, `REG`, `REG` | This instruction `add` takes 3 parameters: 3 registers. The third parameterc the result of the addition of the first and the second parameters. Example: `add r2,r3,r5` sums `r2` and `r3` and stores the result into `r5` | 10 | Oui | Oui | - |
| 5 | `sub` | `REG`, `REG`, `REG` | Même que add mais soustraitOi | Oui | - |
| 6 | `and` | `REG`&#124;`DIR`&#124;`IND`, `REG`&#124;`DIR`&#124;`IND`, `REG` | p1 & p2 -> p3 le paramètre 3 et toujours un registre. Cette opération modifie le carry. `and r2,%0,r3` met r2 & 0 dans r3 | 6 | Oui | Oui | 4 |
| 7 | `or` | `REG`&#124;`DIR`&#124;`IND`, `REG`&#124;`DIR`&#124;`IND`, `REG` | Même que and mais avec le ou (&#124; binaire).| 6 | Oui | Oui | 4 |
| 8 | `xor` | `REG`&#124;`DIR`&#124;`IND`, `REG`&#124;`DIR`&#124;`IND`, `REG` | Même que and mais avec le ou exclusif (^ binaire).| 6 | Oui | Oui | 4 |
| 9 | `zjmp` | `DIR` | Cette instruction n’est pas suivie d’octet pour décrire les paramètres. Elle prend toujours un index (`IND_SIZE`) et fait un saut a cet index si le carry est à un. Si le carry est nul, zjmp ne fait rien mais consomme le même temps. `zjmp %23` met si carry == 1 met `(PC + (23 % IDX_MOD))` dans le PC. | 20 | Non | Non | 2 |
| 10 | `ldi` | `REG`&#124;`DIR`&#124;`IND`, `REG`&#124;`DIR`, `REG` | Cette opération modifie le carry. `ldi 3,%4,r1` lit `IND_SIZE` octets à l’adresse : `(PC + (3 % IDX_MOD))` ajoute 4 à cette valeur. On nommera S cette somme. On lit `REG_SIZE` octets a l’adresse `(PC + (S % IDX_MOD))` qu’on copie dans r1. Les paramètre 1 et 2 sont des index.| 25 | Non | Oui | 2 |
